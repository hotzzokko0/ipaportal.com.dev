...
?>
<p class="hidden visible-xs"><label class="control-label">SNS 동시등록</label></p>
...
if ($config['cf_facebook_appid']) {
    ...
    echo '<li>';
    if ($facebook_user) {
        echo '<img src="'.G5_SNS_URL.'/icon/facebook.png" id="facebook_icon"></li><li class="checkbox-middle">';
        echo '<label for="" class="sr-only sound_only">페이스북 동시 등록</label>';
        ...
        echo '<a href="'.$facebook_url.'" id="facebook_url" onclick="return false;"><img src="'.G5_SNS_URL.'/icon/facebook'.($facebook_user?'':'_off').'.png" id="facebook_icon"></a></li><li class="checkbox-middle">';
        echo '<label for="" class="sr-only sound_only">페이스북 동시 등록</label>';
...
if ($config['cf_twitter_key']) {
    ...
    echo '<li>';
    if ($twitter_user) {
        echo '<img src="'.G5_SNS_URL.'/icon/twitter.png" id="twitter_icon"></li><li class="checkbox-middle">';
        echo '<label for="" class="sr-only sound_only">트위터 동시 등록</label>';
        echo '<input type="checkbox" name="twitter_checked" id="twitter_checked" '.(get_cookie('ck_twitter_checked')?'checked':'').' value="1">';
    } else {
        echo '<a href="'.$twitter_url.'" id="twitter_url" onclick="return false;"><img src="'.G5_SNS_URL.'/icon/twitter'.(!$twitter_user?'':'_off').'.png" id="twitter_icon"></a></li><li class="checkbox-middle">';
        echo '<label for="" class="sr-only sound_only">트위터 동시 등록</label>';
