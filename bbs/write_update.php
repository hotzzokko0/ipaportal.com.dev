...
if ($w == '' || $w == 'r') {
    ...
    if ($member['mb_id']) {
        ...
        $wr_email = get_email_address(trim($_POST['wr_email'])) ? get_email_address(trim($_POST['wr_email'])) : addslashes($member['mb_email']);
        $wr_homepage = clean_xss_tags($wr_homepage) ? clean_xss_tags($wr_homepage) : addslashes(clean_xss_tags($member['mb_homepage']));
    } else {
        $mb_id = '';
