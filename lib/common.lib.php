...
function get_category_option($bo_table='', $ca_name='', $listExclude = [])
{
    ...
    for ($i=0; $i<count($categories); $i++) {
        ...
        if ($listExclude && in_array($category, $listExclude, true)) {
            continue;
        }
...
function getTypeOptions($_type, $listExcluded = [])
{
    $listType = getTypeText();

    $return = '';

    foreach ((array)$listType as $value => $text) {
        if ($listExcluded && (
                in_array($value, $listExcluded, true) || in_array($text, $listExcluded, true)
            )
        ) {
            continue;
        }
...
function getTypeText($_type = null)
{
    $listType = ['F' => '베이직무료', 'A' => 'A타입(유료)', 'B' => 'B타입(유료)'];
...
function sql_set_charset($charset, $link=null)
{
    ...
    if(function_exists('mysqli_set_charset') && G5_MYSQLI_USE)
        ...
        sql_query(" set names {$charset} ", $link);
}
